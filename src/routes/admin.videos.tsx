...
function AdminVideos() {
  ...
  const save = useMutation({
    mutationFn: async (v: V) => {
      ...
      if (v.id) {
        await updateDoc(doc(db, "videos", v.id), v);
      } else {
        const { id: _, ...rest } = v;
        await addDoc(collection(db, "videos"), rest);
      }
    },
    onSuccess: () => {
            ...
            return (
              <div key={v.id} className="rounded-xl border border-border bg-card overflow-hidden">
                <div className="aspect-video bg-black">
                  {id && (
                    <img src={v.thumbnail_url || `https://img.youtube.com/vi/${id}/hqdefault.jpg`} alt={v.title} className="h-full w-full object-cover" />
                  )}
                </div>
