...
type V = { id?: string; title: string; description: string; video_url: string; thumbnail_url: string; sort_order: number; is_active: boolean };
const empty: V = { title: "", description: "", video_url: "", thumbnail_url: "", sort_order: 0, is_active: true };
...
function AdminVideos() {
  ...
  const save = useMutation({
    mutationFn: async (v: V) => {
      ...
      if (v.id) { const { error } = await supabase.from("videos").update(v).eq("id", v.id); if (error) throw error; }
      else { const { id: _, ...rest } = v; const { error } = await supabase.from("videos").insert(rest); if (error) throw error; }
    },
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["videos"] }); setEditing(null); toast.success("Saved"); },
  ...
  return (
    <AdminLayout title="Videos">
      <div className="flex justify-between mb-4">
        ...
        <Button variant="brand" onClick={() => setEditing(empty)}><Plus className="h-4 w-4 mr-1" /> Add Video</Button>
      </div>
            ...
            return (
              <div key={v.id} className="rounded-xl border border-border bg-card overflow-hidden">
                <div className="aspect-video bg-black">
                  {id && <img src={v.thumbnail_url || `https://img.youtube.com/vi/${id}/hqdefault.jpg`} alt={v.title} className="h-full w-full object-cover" />}
                </div>
                <div className="p-4 flex items-start justify-between gap-2">
                  <div className="min-w-0">
                    ...
                    {!v.is_active && <span className="inline-block mt-2 text-xs bg-muted px-2 py-0.5 rounded">Hidden</span>}
                  </div>
                  <div className="flex gap-1">
                    <Button size="icon" variant="ghost" onClick={() => setEditing(v as V)}><Pencil className="h-4 w-4" /></Button>
                    <Button size="icon" variant="ghost" onClick={() => confirm(`Delete "${v.title}"?`) && del.mutate(v.id!)}><Trash2 className="h-4 w-4" /></Button>
                  </div>
                </div>
              </div>
            );
